...
class Payment(Base):
    ...
    method: Mapped[str] = mapped_column("payment_method", String(50), default=PaymentMethod.CASH.value, nullable=False)
    payment_date: Mapped[date] = mapped_column(nullable=False)

    # Optional tracking
    reference: Mapped[str | None] = mapped_column("reference_number", String(100), nullable=True)  # cheque no, UTR, etc.
